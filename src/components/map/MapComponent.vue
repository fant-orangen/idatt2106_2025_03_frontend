...
function showRouteFor(lat: number, lng: number, poiName: string) {
  ...
  routingControl.value = L.Routing.control({
    ...
    lineOptions: {
      styles: [{ color: '#4a89dc', weight: 4 }],
      extendToWaypoints: true,
      missingRouteTolerance: 0,
    },
    altLineOptions: {
      styles: [{ color: '#4a89dc', weight: 2, opacity: 0.6 }],
    },
    show: true,
...
function createPopupContent(poi: PoiData): string {
  ...
  if (poi.openingHours)
    html += `<strong>${translatedStrings.openingHours}:</strong> ${poi.openingHours}<br>`
  if (poi.contactInfo)
    html += `<strong>${translatedStrings.contact}:</strong> ${poi.contactInfo}<br>`
  ...
  async (newPois) => {
    ...
    setTimeout(() => {
      if (props.userLocation && !userMarker.value && map.value) {
        ...
        userMarker.value = L.marker([props.userLocation.latitude, props.userLocation.longitude], {
          icon: userIcon,
          zIndexOffset: 1000,
        })
          .addTo(map.value)
